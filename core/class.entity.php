...
abstract class Entity{
	...
	public function parseValue($attr_row, $val){
		...
		if(in_array($val,["","0","-"])){throw new Exception("Value '$val' is empty/zero/-");}
		return $val;
	}
	...
	public function selfSource(){ return ""; }
	...
	public function getId(){ return $this->getAttributes()[$this->selfColId()]["value"]; }
	public function getName(){ return $this->getAttributes()[$this->selfColName()]["value"]; }
	...
	public function toValue($attr){
		...
		if($attr["append"] != ""){$val .= " ".$attr["append"];}
		return $val;
	}
	
	public function hasPage(){ return (count($this->getAttributes()) > 2); }
	...
	public static function getEntityRow($e_url){
		...
		if($query->num_rows == 0){throw new Exception("No entities added");}
