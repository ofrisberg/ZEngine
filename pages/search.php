	...
	<script>
		var Ask = {
			...
			onKeyUp: function () {
				...
				var self = this;
				this.timeout = setTimeout(function(){self.search()}, 200);
			},
			setStatus: function(text){this.el.status.innerHTML = text;},
			setResult: function(text){this.el.results.innerHTML = text;},
			addResult: function(text){this.el.results.innerHTML += text;},
			search: function(){
			...
			onSuccess: function(e){
				...
				if(Ask.is_random){Ask.el.question.value = resp.input;Ask.last_value = resp.input;}
				Ask.is_random = false;
			...
			serveResults: function(results){
				if(results.length == 0){return this.setResult("");}
				var html = "<ul class='w3-ul w3-card-4'>";
				for(var i = 0; i < results.length; i++){html += this.resultToString(results[i]);}
				html += "</ul>";
				this.setResult(html);
			},
